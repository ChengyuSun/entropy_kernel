...
def count_star(A,N,neiN,motif,node_occupation):
...
def count_chain(A,N,rest,motif,node_occupation):
    print('开始搜索motif: ',motif)
    n=0
    a = copy.copy(A)
    for i in range(N):
...
def count_poly_edge(A, N,node_occupation):
    print('开始搜索motif： 3')
    n=0
    a = copy.copy(A)
    for i in range(N):
...
def countEdge(A,nodN):
    node_occupation=['0' for i in range(nodN)]
    for i in range(nodN):
        A[i][i] = 0
    print('start counting edges and motifs')
    Nm_1 = count_chain(A, nodN, 2, 1,node_occupation)
    Nm_2 = count_chain(A, nodN, 3, 2,node_occupation)
    Nm_3 = count_poly_edge(A, nodN,node_occupation)
    Nm_4 = count_chain(A, nodN, 4, 4,node_occupation)
    Nm_5= count_star(A, nodN,3,5,node_occupation)
    Nm_6 = count_poly_qua(A, nodN,node_occupation)
    Nm_7 = count_chain(A, nodN, 5, 7,node_occupation)
    Nm_8 = count_star(A,nodN,4,8,node_occupation)
    count_motifs=[Nm_1,Nm_2,Nm_3,Nm_4,Nm_5,Nm_6,Nm_7,Nm_8]
    # with open('/new_disk_B/scy/pub/pub_count_edge.txt', 'w') as file:
    #     for line in node_occupation:
    #         for item in line:
    #             file.write(item+',')
    #         file.write('\n')
    return node_occupation,count_motifs
