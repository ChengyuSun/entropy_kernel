...
def graphEntropy(motifNumber,nodN):
    dN = read_data(nodN - 1, '../entropy/file/devide_1000_Nodes.csv')
...
def graphlet_entropy(n,KT=1000,r=3):
    # print(n)
    # l = [2, 3, 3, 3, 4, 4, 4, 4]
    # e = [1, 2, 2, 3, 3, 3, 3, 3]
    node_num=[3,3,4,4,4,4,4,4]
    edge_num=[3,2,6,5,4,4,3,3]
    # KT=10000
    beta = symbols('BETA')
    DELTA = 256
    pi = 3.1415926
    ...
    for i in range(Nm):
        if n[i]==0:
            ...
            continue
        m_integral = (Integral ** edge_num[i%8]) * r ** (node_num[i%8] - edge_num[i%8])
        logZ = n[i] * (math.log(n[i]) + 1 + log(m_integral) - math.log(factorial(node_num[i%8])))
        U = diff(logZ, beta)
        E = logZ + U * beta
        E = E.subs(beta, 1.0 / (KT))
        Entropy.append(float(E))
    return Entropy